function [idx,centroids,iterations,ifault] = kmedians(x,k,init_centroids,ITER,varargin)
%KMEDIAN is a modification of the K-Means algorithm by Stuart P. Lloyd
%where the median is used instead of the mean to unpdate the centroids.
...
% ifault: 2 = no converged, 1 = empty cluster(s), 0 = converged
    ...
    for T = 1:ITER + 1
        ...
        for ii = 1:k
            elements = find(idx == ii);
            if length(elements) > 1
                centroids(ii,:) = median(x(elements,:));
            elseif length(elements) == 1 %if only 1 element
                centroids(ii,:) = x(elements,:);             
            elseif isempty(elements) %if no elements
                centroids(ii,:) = centroids(ii,:); 
